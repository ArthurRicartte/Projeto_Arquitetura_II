## Projeto de Arquitetura de Computadores II 🖥️

* Implementação de um simulador de uma microarquitetura (baseada em uma versão modificada da máquina **Mic-1**).

## ⚙️ Linguagem de programação utilizada:
* Python

## 👥 Integrantes do Grupo
* MARQUISON DE SOUSA E SILVA JUNIOR
* JOAO VELOSO DA CRUZ GOUVEIA SEGUNDO
* FELIPE RODRIGUES COELHO
* ARTHUR RICARTTE PEREIRA DE SOUZA

## 📌 Informações sobre o Projeto

> Atualmente, a versão mais recente é referente à **Etapa 3**. A branch `main` está alinhada exclusivamente com os requisitos desta terceira etapa.

### 📂 Estrutura de Arquivos Principais

* **`APS1/APS_1.py`**: Arquivo principal que contém toda a lógica de simulação da ULA.
* **`APS1/teste.txt`**: Arquivo que contém as instruções e entradas utilizadas como exemplo para os testes.
* **`APS1/log.txt`**: Arquivo de saída gerado pelo simulador, está alinhado com os arquivos de saída disponibilizados pela professora.
